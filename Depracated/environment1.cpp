// environment.cpp
// Compile with: g++ environment.cpp neural_network.cpp -o simulation -lode -ldrawstuff -lGL -lGLU -lglut
...
// --- Network related definitions (must match neural_network.cpp) ---
#define ACTOR_OUTPUTS 4

extern "C" {
    // Neural network function prototypes from neural_network.cpp
    void initNetwork();
 ...
 * This function checks the z-component of the body's “up” vector.
 ******************************************************************************/
static int hasFallen(Quadruped *quad)
{
    const dReal *R = dBodyGetRotation(quad->body);
    double up_z = R[10]; // The 11th element is the z-component of the up vector.
    return (up_z < 0.6);
}
...
static void updateSensorsAndControl(Quadruped *quad)
{
    ...
    switch (state) {
        ...
        case 1:
            ...
            dJointSetHingeParam(quad->wheelJoint[1], dParamVel, 40);
            dJointSetHingeParam(quad->wheelJoint[3], dParamVel, 40);
            break;
        case 2:
            dJointSetHingeParam(quad->wheelJoint[0], dParamVel, 40);
            dJointSetHingeParam(quad->wheelJoint[2], dParamVel, 40);
        ...
        case 3:
            for (int i = 0; i < 4; i++) {
                dJointSetHingeParam(quad->wheelJoint[i], dParamVel, 20);
            }
            break;
        default:
            break;
